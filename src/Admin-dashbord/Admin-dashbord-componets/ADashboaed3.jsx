...
import Admintable_Paidusers from "../Admin-table/Admintable_Paidusers";
...
const ADashboard3 = () => {
  ...
  const fetchAllUsers = async () => {
    try {
        ...
        console.log('Response for users data:', response.data);

        // Access the 'message' property to get the users array
        const users = response.data.message || response.data;

        if (Array.isArray(users)) {
            // Filter users who have a Payment associated with them
            const usersWithPayments = users.filter(user => user.Payment !== null);

            // Log the filtered users for debugging
            console.log('Filtered Users with Payments:', usersWithPayments);

            // Set the filtered users with payments
            setUsersdata(usersWithPayments);

            console.log('Total Users with Payments:', usersWithPayments.length);
  ...
  return (
    <>
      ...
      <div className="Admin-list-table-box">
        <Admintable_Paidusers filteredUsers={filteredUsers} />
      </div>
