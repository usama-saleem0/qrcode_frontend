...
const EventFullscreen = () => {
  ...
  const fetchQRtepmlete = async (id) => {
    try {
      const response = await axios.get(`${baseurl}/getqrtempleteventById/${id}`);
            ...
            <div className='templetes1' style={{ backgroundImage: `url(${qrtemplete.selectedCover || rain})`, }}>
              ...
              <div className='grad hover-grad' style={{ background: `transparent ` }}>
                ...
                <div className='setlogs hero-li-list-box'>
                  ...
                  <div className='hell-box-inside hitx' style={{ background: ` ${qrtemplete.bottom}`, borderRadius: '0px' }}>
                    ...
                    <h3>{qrtemplete.DescriptionEvent || "We want to invite you to our wedding and enjoy a very special day together."}</h3>

                    <a href={qrtemplete.ButtonURl}>
                      <button className='byn-box-in' style={{ background: ` ${qrtemplete.top}` }}> {qrtemplete.ButtonDescription || "Confirm assistance"}</button>
                    </a>
