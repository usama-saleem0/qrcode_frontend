...
const Creatqrcode = ({ qrCodeData, setQRCodeData, setLoading }) => {
  // const [urls, setUrsl] = useState('https://fffd-2407-aa80-15-d9a9-3550-c2-8894-624a.ngrok-free.app/qrscan');
  const [urls, setUrsl] = useState('https://qrn1.com/qrscan');
  const [socail, setSocail] = useState('');
  const [WholeQr, setWholeQr] = useState('');
  const [wdth, setwdth] = useState(200);
  ...
  const [qrtemplate, setQrTemplates] = useState([]);
  ...
  const [qrcodetypes, setQrType] = useState(false);





  console.log('gotisssssst', qrCodeData)
  ...
  function getImageFileObject(imageFile) {
    const file = imageFile.file; // Access the actual File object
    if (file instanceof File) {
      const imageURL = URL.createObjectURL(file);
      setSocail(imageURL);
      console.log('Image uploaded:', imageURL);
    } else {
      console.error("Provided image is not a File object.");
    }
  }


  const handleCheckboxChangeTemplete = (e) => {
    setIsChecked(e.target.checked);
  };

  const handleSubmitTemplete = async (qrCodeUrl) => {
    if (!isChecked) return; 
    console.log(qrCodeUrl, "formDatassss");
    const QrCode = qrCodeUrl; // Only pass the URL directly here for testing
    ...
    try {
      // Log values individually to detect issues
      console.log({ userId, Name, wdth, margin, qrStyle, color, bordercolor });

      const response = await axios.post(`${baseurl}/createQrTemplate`, {
        QrCode,
        Name,
        userId,
        wdth,
        ...
        gradborderinput1,
        gradborderinput2,
        gradbordercornerinput1,
        gradbordercornerinput2,
        bordersquareconercolor,
        borderdotconercolor,
        gradbackground1,
        gradbackground2,
        gradicon1,
      ...
      if (error.response) {
        ...
        toast.error('No response received from the server. Please check your network.');
      } else {
  ...
  const setQrs = (e, wi, mar, marleft, bottom, hei) => {
  ...
  // const setQrss = (e, wi, hei, mar, marleft, color, bordercolor, level, socail) => {
  ...
  //   console.log("levelsssssssss", level)

  // }

  const setQrss = (e, wi, hei, mar, bordercolor, marleft, color, level, socail) => {
    console.log("qrCode values:", { e, wi, hei, mar, bordercolor, marleft, color, level, socail });

    // Set state with the values
    setWholeQr(e);
    setwdth(wi);
    setqrheight(hei);  // Ensure this value isn't undefined or use a default value
    setmargin(mar);
    setmarginleft(marleft);
    setLevel(level);
    setColor(color);
  ...
  const [urlid, setId] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    const selectname = localStorage.getItem('name');

    console.log('idss', urlid)

    // if(urlid){
    // const fetch = `${urldata}/${urlid}`
    ...
    const fetch = urlid && qrcodetypes != 'Website' ? `${urldata}/${urlid}` : `${urldata}`;

    console.log(userId, "userId");


    const hyperId = localStorage.getItem('hyperId');

    const QrhyperId = localStorage.getItem('QrhyperId');
    ...
    if (QrhyperId) {
      try {
        const response = await axios.post(`${baseurl}/updateQrCode/${QrhyperId}`, {
          QrCode,
          Name,
          selectname,
          userId,
        ...
        localStorage.setItem('QrhyperId', '');
        localStorage.setItem('hyperId', '');
  ...
  const fetchQRCodes = async () => {
    ...
    try {
      ...
      console.log(response.data.data); // Ensure you're accessing the correct array
  ...
  const calculateSize = (width, height) => {
    if (window.innerWidth >= 1920) {
      return { width: width, height: height };  // Use larger sizes for wider screens
    } else if (window.innerWidth >= 1200) {
      return { width: width * 0.8, height: height * 0.8 };  // Scale down for medium screens
    } else {
      return { width: width * 0.6, height: height * 0.6 };  // Scale down further for smaller screens
    }
  };
  ...
  // const handleButtonClick = (svgString, width, qrheight, margin) => {

  //   console.log('margin', margin, "width", width, "height", qrheight)
  //   setwdth(width);
  //   setqrheight(width)
  //   setQrs(svgString, width, qrheight, margin);
  // };


  // const handleButtonClick = (svgString, width, qrheight, margin) => {
  //   console.log('margin', margin, "width", width, "height", qrheight , window.innerWidth);

  //   // Check window width and set appropriate values
  //   let adjustedWidth = width;
  //   if (window.innerWidth >= 1920) {
  //     adjustedWidth = width * 1;  // Adjust the width for large screens
  //   }
  //   else if (window.innerWidth >= 1280) {
  //     adjustedWidth = width * 1;  
  //   }

  //   // Update the state for width and height
  //   setwdth(adjustedWidth);
  //   setqrheight(adjustedWidth);

  //   // Set QR code properties after state update
  //   setQrs(svgString, adjustedWidth, qrheight, margin);
  // };

  const handleButtonClick = (svgString, width, qrheight, margin, marginleft, basit) => {
    console.log("width", width, 'margin', margin, "marginleft ", marginleft, "height", qrheight, window.innerWidth);
    ...
    if (window.innerWidth >= 1920) {
      ...
      console.log("WARNING123", basit)
      scaleWidth = 1;
      scaleHeight = 1;
      scaleMargin = 2;
      if (marginleft === 'btnlast') {
        console.log("WARNING")
        marginleft = -70
    ...
    // Adjust the values for width, height, and margin based on the scale
    const adjustedWidth = width * scaleWidth;
    const adjustedHeight = qrheight * scaleHeight;
    // const adjustedMargin = margin * scaleMargin;
    const adjustedMargin = margin;
    const adjustedMarginleft = marginleft;

    setwdth(adjustedWidth);
    setqrheight(adjustedWidth);
    setmarginleft(adjustedMarginleft);

    // Set QR code properties after state update
    setQrs(svgString, adjustedWidth, adjustedHeight, adjustedMargin, adjustedMarginleft);
  };
  ...
  useEffect(() => {
    ...
    if (qrCode) {
      qrCode.update({
        ...
        data: generateQRCodeData(), // Update the QR code data
        imageOptions: {
  ...
  }, [qrStyle, qrheight, wdth, marginleft, qrStyleCorners, qrStyledotCorners, socail, level, gradborderinput1, gradborderinput2, gradbordercornerinput1, gradbordercornerinput2, bordersquareconercolor, borderdotconercolor, gradbackground1, gradbackground2, gradicon1, gradicon2, color, bordercolor, qrCode]);
  ...
  useEffect(() => {
    const qrCodeInstance = new QRCodeStyling({
      ...
      height: qrheight,
      image: socail,
      data: generateQRCodeData(), // Set initial data 
      dotsOptions: {
        type: qrStyle, // Change the dots style dynamically
        color: bordercolor || "#000000", // Use the dynamic foreground color
        ...(useGradientsicons
          ...
          <div className="main-content--b--flex">
            <div className="main-content--b--website">
                ...
                <div className="pdf-box-search">
                  <div className="ui-pdf-cheack-box">
                    <input type="checkbox" class="ui-checkbox" 
                      checked={isChecked}
                      onChange={handleCheckboxChangeTemplete}
                    />
                  </div>
              ...
              <div className="fram-box">
                ...
                {/* {qrtemplate.map((qrCode) => (

                  <button onClick={() => setQrss(qrCode.qrcode, qrCode.wdth, qrCode.qrheight, qrCode.margin, qrCode.color, qrCode.bordercolor, qrCode.level, qrCode.socail)}>
                    <img src={qrCode.qrcode} alt="" /></button>
                ))} */}
                  ...
                  <button
                    onClick={() =>
                      setQrss(
                        ...
                        qrCode.wdth,          // qrCode.wdth
                        qrCode.qrheight || 300, // Default to 100 if undefined
                        qrCode.margin,        // qrCode.margin
                        qrCode.bordercolor,   // qrCode.bordercolor
                        qrCode.marleft,       // qrCode.marleft (correct name?)
                        qrCode.color,         // qrCode.color
                        qrCode.level,         // qrCode.level
                        qrCode.socail 
                      )
                    }
                  >
                  ...
                  <AccordionItem>
                    ...
                    <AccordionItemPanel>
                      <div className="fram-box">
                        ...
                        {/* width, margin-top, height, marginLeft */}

                        <button onClick={() => handleButtonClick(svg1String, 140, 20, 10)}>
                          <img src={btn4} alt="" />
                        </button>
                        <button onClick={() => handleButtonClick(svg2String, 140, 20, 10)}><img src={btn4} alt="" /></button>

                        <button onClick={() => handleButtonClick(svg3String, 140, 20, 10)}><img src={btn5} alt="" /></button>

                        <button onClick={() => handleButtonClick(svg4String, 140, 33, 10, 'btnother')}><img src={btn11} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg5String, 140, 33, 0, 10, 'btnother')}><img src={btn8} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg1String, 140, 33, 0, 10, 'btnother')}><img src={btn9} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg6String, 140, 33, 0, 10, 'btnother')}><img src={btn10} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg7String, 140, 53, 0, 160, 'btnother')}><img src={btn7} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg8String, 140, 39, 10, 'btnother')}><img src={btn12} alt="" /></button>


                        <button onClick={() => handleButtonClick(svg9String, 140, 39, 15, 'btnother')}><img src={btn15} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg16String, 125, 54, 2, 'btnother')}><img src={btn19} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg10String, 125, 76, -30, 'btnother')}><img src={btn17} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg11String, 130, 65, 0, 13, 'btnother')}><img src={btn14} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg12String, 130, 37, 0, 35, 'btnother')}><img src={btn20} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg13String, 130, 50, 0, 50, 'btnother')}><img src={btn21} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg14String, 130, 57, 30, 'btnother')}><img src={btn18} alt="" /></button>
                        <button onClick={() => handleButtonClick(svg15String, 120, 13, -70, 'btnlast')}><img src={btn16} alt="" /></button>
                ...
                <div className="whiteback" style={{ position: 'relative' }} ref={whiteback}>
                  ...
                  {qrCode ? (
                    ...
                    >
                      ...
                      <div className='hello-1-1' style={{ position: 'absolute', bottom: bottomposition, color: textcolors, fontWeight: 'bold' }}>
                        {textcolor}
                      </div>
