...
const NewQR_Link = () => {
  ...
  const fetchListQr = async (id) => {
   
    try {
      const response = await axios.get(`${baseurl}/getListQr/${id}`);
      ...
      settops(response.data.data.top)
  ...
  const handleSubmit1 = async (e) => {
   ...
   if(id){
    ...
    try {
      const response = await axios.post(`${baseurl}/updatenewqrtemplete/${id}`, {
        selectedCover,
        selectedImage,
        color,
        descriptionsize,
